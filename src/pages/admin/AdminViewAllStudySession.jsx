 ...
 const handleApproved = (id) => {
  ...
  <h1 className='text-3xl font-bold text-center mb-6'>All Study Session</h1>
    <div className='w-full mx-auto mb-7'>
          {
            session.map((item) => <div key={item._id}>
            <div className='bg-base-200 p-4 rounded-xl mb-5'>
    <h3 className="courseCard-title text-center">{item.sessionTitle}</h3>
    <p className="courseCard-description text-center">{item.sessionDescription}</p>
    <p className="text-lg font-semibold text-gray-800 text-center">Tutor Name :<span className='text-gray-500'>{item.tutorName}</span></p>
    ...
    <div className='md:flex justify-evenly items-center gap-5'>
    <p className="text-lg  font-semibold text-gray-800">Registration Fee :<span className='text-gray-500'>{item.registrationFee}</span></p>
    <p className="text-lg font-semibold text-gray-800 text-center">Status :<span className='text-gray-500'>{item.status}</span></p>
    </div>
    <div className="card-actions">
    {
        item.status === 'pending' ? 
        <>
         <div className="dropdown dropdown-hover w-5/12 md:w-3/12 lg:w-2/12 mx-auto">
